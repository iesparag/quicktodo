 ...
 * Throws for IO errors
 * @param {string} description
 * @returns {Promise<Object>} created task {id, description, status, createdAt}
 */
export async function addTask(description) {
  ...
  try {
    ...
    throw new Error('Could not load existing tasks: ' + err.message);
  }
  tasks.push(task);
  try {
    await saveTasks(tasks);
  } catch (err) {
    throw new Error('Could not save new task: ' + err.message);
  }
  return task;
}
